...
namespace WhiteBit.Net.Interfaces.Clients.V4Api
{
    ...
    public interface IWhiteBitSocketClientV4Api : ISocketApiClient, IDisposable
    {
        ...
        /// <param name="ct">Cancellation token for closing this subscription</param>
        /// <returns>A stream subscription. This stream subscription can be used to be notified when the socket is disconnected/reconnected</returns>
        Task<CallResult<UpdateSubscription>> SubscribeToUserTradeUpdatesAsync(IEnumerable<string> symbols, Action<DataEvent<IEnumerable<WhiteBitUserTradeUpdate>>> onMessage, CancellationToken ct = default);
        ...
        /// <param name="ct">Cancellation token for closing this subscription</param>
        /// <returns>A stream subscription. This stream subscription can be used to be notified when the socket is disconnected/reconnected</returns>
        Task<CallResult<UpdateSubscription>> SubscribeToPositionUpdatesAsync(Action<DataEvent<WhiteBitPositions>> onMessage, CancellationToken ct = default);
